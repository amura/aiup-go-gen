...
func NewChatManager(agents []agent.Agent, prompts map[string]string, selector StateSelector) *ChatManager {
	return &ChatManager{
		agents:   agents,
		prompts:  prompts,
		selector: selector,
...
func (cm *ChatManager) Start() {
	...
	go func() {
		...
		for {
			...
			nextIdx := cm.selector.NextAgent(cm.lastAgentIdx, cm.agents, msg, cm.history, cm.context)
